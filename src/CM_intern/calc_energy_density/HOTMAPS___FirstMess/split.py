...
country_file_list = []
for (dirpath,dirnames,filenames) in os.walk(building_lyr_base_path):
	country_file_list.extend(filenames)
...
def get_building_def_shape_file(building_lyr_file_in, building_lyr_path_out, buildings_input_feat):
	"""
	prepares building lyr file
	"""
	input_base_name = os.path.basename(building_lyr_file_in)
	country_id = input_base_name.split("-")[0]
	building_lyr_path_out2 = "%s/%s/" % (building_lyr_path_out, country_id)
	
	

	process_any_file = False
	if not os.path.exists(building_lyr_file_in):
		print("Building lyr input file doesn't exist: %s" % building_lyr_file_in )
		return False
	
	mod_time_in = os.stat(building_lyr_file_in).st_mtime
	#unzip Files
	with closing(zipfile.ZipFile(building_lyr_file_in)) as zfile:
		for info in zfile.infolist():
			if lyr_zip_name in info.filename:
				print "Extract: %s" % info.filename
	...
	if process_any_file == True or not os.path.exists(building_lyr_file_out3):
		# Transformation of Layer CRS
		rp.reprojection(building_lyr_file_out2, building_lyr_file_out3, buildings_input_feat)
		NEW_SHAPE_FILE = True
