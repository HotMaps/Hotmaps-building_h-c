...
#         dict_ = dict(zip(unique, counts))
        ...
        sparseRow,sparseCol = np.nonzero(blobs_labels1)

        data1 = blobs_labels1[sparseRow , sparseCol]
        data2 = g[sparseRow , sparseCol]
        
        
        data = np.asarray(sorted(zip(sparseRow,sparseCol,data1,data2),key=lambda x:x[2]))
        unique, counts = np.unique(data1, return_counts=True)
        end = np.cumsum(counts)
        st = np.concatenate((np.zeros((1)), end[0:numLabels-1]+1))
        #dict_ = dict(zip(unique, counts))
        
        sum_ = np.zeros((numLabels+1))
        
        #sum_[:,] = np.sum(data[st:end,3]) 
        
        for i in range(numLabels):
            pot = np.sum(data[st[i]:end[i],3])
            if pot>=DH_threshold:
...
#                 test[i]=1
    ...
    #             dict_[i]=i
        ...
        DH = (test[blobs_labels1]).astype(bool)
        #g = (sum_[blobs_labels1]).astype(float)
...
#         miny = maxy + transform[5] * cutRastDatasource.RasterYSize
    ...
    DH = arr1>pix_threshold
    (dimX,dimY) = DH.shape
    result = np.zeros((dimX,dimY)).astype(bool)
    ...
    for fid in range(inLayer.GetFeatureCount()):
        ...
        #updatedDH, updatedg = potDHAreas(arr_out1,arr_out2, rasterOrigin2,fid,0.1,10)
        updatedDH = potDHAreas(arr_out1,arr_out2, rasterOrigin2,fid,0.1,10)
        result[lowIndexX:upIndexX,lowIndexY:upIndexY] += updatedDH
        arr_out1 = None
        arr_out2 = None
        inFeature = None
    
    outRasterPath = "/home/simulant/ag_lukas/personen/Mostafa/potDH/result3.tif"
    array2raster(outRasterPath,rasterOrigin,100,-100,"int8",result.astype(int),0)
